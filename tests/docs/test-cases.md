...
| P1-OBS-006 | Observability | Java admin observability service excludes unreported provider cache calls from provider cache denominators, requires same-row LLM call filters for trace queries, keeps list aggregates scoped to filtered LLM calls, enforces `AI_OBSERVABILITY_RAW_READ` before selecting raw text, writes audit logs for permitted raw reads without exposing the other raw side, returns high-consumption stats, and exposes non-raw standalone LLM call detail | Unit/Mapper | P1 | Yes | `ai_interviewer_admin/src/test/java/com/aiinterviewer/admin/observability/AiObservabilityServiceTest.java` | `cd ai_interviewer_admin && env JAVA_HOME=$HOME/.jenv/versions/21 PATH=$HOME/.jenv/versions/21/bin:$PATH mvn -Dtest=AiObservabilityServiceTest test` | Conditional | Regression, Admin observability, Security |
