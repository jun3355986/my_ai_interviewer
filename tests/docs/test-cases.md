...
| P1-INTERVIEW-002 | Interview | Interview replay tooling parses trace JSONL, validates SSE event order, and reports missing expected events or stages | API/Tooling | P1 | Yes | `tests/api/pytest/test_interview_replay_tooling.py` | `cd ai_interviewer && uv run python -m pytest ../tests/api/pytest/test_interview_replay_tooling.py -q` | Conditional | Regression, Replay, SSE |
| P1-INTERVIEW-003 | Interview | Python AI SSE stub returns deterministic `status/chunk/result/done` events for the first chat turn without calling a real LLM provider | API/Stub | P1 | Yes | `tests/stubs/python-ai/app.py`, `tests/api/pytest/test_interview_replay_tooling.py` | `cd ai_interviewer && uv run python -m pytest ../tests/api/pytest/test_interview_replay_tooling.py -q` | Conditional | Regression, Stub AI, SSE |
