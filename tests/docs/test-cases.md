...
| P1-OBS-006 | Observability | Java admin observability service excludes unreported provider cache calls from provider cache denominators, constrains stats LLM-call aggregates by call type/provider/model filters, writes audit logs for requested raw reads without exposing the other raw side, filters traces by trace ID/call type, returns high-consumption stats, and exposes non-raw standalone LLM call detail | Unit | P1 | Yes | `ai_interviewer_admin/src/test/java/com/aiinterviewer/admin/observability/AiObservabilityServiceTest.java` | `cd ai_interviewer_admin && env JAVA_HOME=$HOME/.jenv/versions/21 PATH=$HOME/.jenv/versions/21/bin:$PATH mvn -Dtest=AiObservabilityServiceTest test` | Conditional | Regression, Admin observability |
