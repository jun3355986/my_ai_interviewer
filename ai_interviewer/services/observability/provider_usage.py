...
def _non_negative_int_token(value: Any) -> int | None:
...
def normalize_provider_usage(
    ...
    if usage is None:
        prompt_tokens = _non_negative_int_token(estimated_prompt_tokens)
        completion_tokens = _non_negative_int_token(estimated_completion_tokens)
        total_tokens = None
    ...
    prompt_tokens = _non_negative_int_token(raw_usage.get("prompt_tokens"))
    completion_tokens = _non_negative_int_token(raw_usage.get("completion_tokens"))
    total_tokens = _non_negative_int_token(raw_usage.get("total_tokens"))
    cache_hit_tokens = None
    cache_miss_tokens = None

    if provider.lower() == "deepseek":
        cache_hit_tokens = _non_negative_int_token(
            raw_usage.get("prompt_cache_hit_tokens")
        )
        cache_miss_tokens = _non_negative_int_token(
            raw_usage.get("prompt_cache_miss_tokens")
        )
    else:
        prompt_details = raw_usage.get("prompt_tokens_details")
        if isinstance(prompt_details, Mapping):
            cached_tokens = _non_negative_int_token(prompt_details.get("cached_tokens"))
